...
import edu.bonn.mobilegaming.geoquest.contextmanager.*;
...
public class ContextManagerTests {
	...
	public void testStaticContext() {
		...
		assertEquals(staticContext.getAge(), context.getAge());
		assertEquals(staticContext.getActivLevel(), context.getActivLevel());
		assertEquals(staticContext.getKnowledgeLevel(), context.getKnowledgeLevel());
		assertEquals(staticContext.getTechLevel(), context.getTechLevel());
...
//	@Test
	public void testHistoryContent() {
		...
		cManager.setEndValues(missionId, 50);
		SystemClock.setCurrentTimeMillis(midTime);
		cManager.setStartValues(missionId);
		SystemClock.setCurrentTimeMillis(endTime);
		cManager.setEndValues(missionId, 0);
		...
		assertEquals(gameDuration, LoggingContext.getGameDuration());
		
		MissionContext mContext = missionHistory.get(0).getDynamicContext();
		assertNotNull(mContext);
		assertEquals(50, mContext.getResult());
		assertEquals(midTime - startTime, mContext.getMissionDuration());
		assertEquals(midTime - startTime, mContext.getElapsedTime());	
		...
		assertNotNull(mContext);
		assertEquals(0, mContext.getResult());
		assertEquals(endTime - midTime, mContext.getMissionDuration());
		assertEquals(endTime - startTime, mContext.getElapsedTime());	
	...
	public void testSetEndValuesUnknownMissionId() {
		...
		cManager.setEndValues("Mission_2", 50);
	}
	...
	public void testSetEndValuesTwice() {
		ContextManager cManager = new ContextManager(applicationContext);
		cManager.setStartValues("Mission_1");
		cManager.setEndValues("Mission_1", 50);
		cManager.setEndValues("Mission_1", 50);
	}
	...
	public void testRemainingGameTime() {
		...
		assertEquals(100, cManager.getRemainingGameTime());	
		SystemClock.setCurrentTimeMillis(50);
		assertEquals(50, cManager.getRemainingGameTime());
	}
