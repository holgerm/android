...
public class Mission implements Serializable {
	...
	// Set the Intent which is later used to start this Missions Activity.
	private void createStartingIntent(String id2) {
		try {
			if (useWebLayout) {
				startingIntent = new Intent(getMainActivity(),
						Class.forName(PACKAGE_BASE_NAME + "WebTech"));
			} else {
				startingIntent = new Intent(getMainActivity(),
						Class.forName(PACKAGE_BASE_NAME + missionType));
			}
		} catch (ClassNotFoundException e) {
			Log.d(LOG_TAG, " invalid type specified; command not found: "
					+ missionType);
			e.printStackTrace();
		}
		startingIntent.putExtra("missionID", id);
	}

	private void loadXML(Handler loadHandler) {
		missionType = xmlMissionNode.attributeValue("type");
		chooseMissionLayout();
		createStartingIntent(id);
		setCancelStatus();
		createRules();
	...
	public String toString() {
		return missionType + " id=" + id + "; " + super.toString();
	}
