...
//		if (isGameActivity(newActivityOfThisApp) && newActivityOfThisApp instanceof MissionOrToolActivity)
//			setCurrentActivity((MissionOrToolActivity)newActivityOfThisApp);
	...
	/**
	 * Plays a sound file either blocking or non-blocking regarding the user
	 * interaction options on the currently active mission or tool.
	 * 
	 * @param path
	...
	public static boolean playAudio(String path, boolean blocking) {
		...
		try {
			...
			Log.e(TAG, "Could not start Media Player.");
			return false;
		}
	...
	public static Bitmap loadBitmap(String relativeResourcePath, boolean scale) {
		...
		Bitmap bitmap = BitmapUtil.readBitmapFromFile(bitmapFile, getContext());
		if (scale)
			bitmap = BitmapUtil.scaleBitmapToScreenWidth(bitmap, getContext());
		return (bitmap);
	}
