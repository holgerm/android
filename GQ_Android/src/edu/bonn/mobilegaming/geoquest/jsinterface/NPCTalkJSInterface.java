...
/**
 * This interface is the connection between the JS-File for the NPCTalk and GeoQuest.
 * 
 * In the JS-File it gives the possibility to access XML values and to pass values 
 * back to GeoQuest. 
 */
public class NPCTalkJSInterface {
	...
	// TODO -- Sabine -- WebTech �bergabe der Dialoge an JS �ndern. Diese L�sung ist schlecht.
	/**
		...
		while (it.hasNext()) {
			...
			if (it.hasNext()) { // Nextbutton
				if (item.nextDialogButtonText != null) {
					...
					if (buttons[i] == null) {
						buttons[i] = webTechView.getResourceString(R.string.npctalk_nextDialogButtonText);
					}
				}
			} else { // Endbutton
				buttons[i] = mission.xmlMissionNode.attributeValue("endbuttontext");
				if (buttons[i] == null) {
					buttons[i] =  webTechView.getResourceString(R.string.default_endButtonText);
				}
			}
			++i;
		}
